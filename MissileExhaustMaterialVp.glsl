...
void main()
{
	...
	vec3 dir = vec3(0, -1, 0); // Move the particle downward
	
	// Define time in a cyclic manner
    float phase = two_pi*particle_id; // Start the sin wave later depending on the particle_id
	float param = timer / 0.5 + phase; // The constant that divides "timer" adjusts the "speed" of the fire
    ...
    float t = abs(circtime)*(0.4 + abs(normal.y)); // Our time parameter
	...
	position += dir*accel*t*t; // Particle moves down
